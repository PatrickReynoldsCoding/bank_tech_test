class Account {
  ...
  statement () {
    ...
    this.transactions.map(transaction => {
      ...
      statementBody = statementBody + `${transaction.date} || ${transaction.amount} || || ${transaction.balanceAfterTransaction}\n`
      } else {
      statementBody = statementBody + `${transaction.date} || || ${transaction.amount*-1} || ${transaction.balanceAfterTransaction}\n`
      }
    })
